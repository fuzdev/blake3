...
| `patch_npm_package.ts`  | `build:wasm:web`, `build:wasm:small:web`   | Generates `stream.js` (from `blake3_wasm_core/stream.ts`), `index.js` (Node auto-init), `browser.js` (guarded exports), `index.d.ts`, npm README; patches `package.json` with the kebab-case npm `name` (wasm-pack emits the crate name), metadata, exports map, and `sideEffects: ['./index.js']` (protects the Node entry's auto-init from tree-shaking) |
